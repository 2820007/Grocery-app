...
//Low stock alert send to admin email
const checkLowestStock = inngest.createFunction(
  {
    id: "check-low-stock",
...
//Monthly offers email (1st every month-payday)
...
const sendMonthlyOffers = inngest.createFunction(
  ...
  async ({ step }) => {
    const { deals, users } = await step.run(
      ...
      async () => {
        //Get discounted products as featured deals
...
const autoAssignRider=inngest.createFunction({
    ...
    const result=await step.run("assign-rider",async()=>{
        ...
        // skip  if the order doesn't exist, already assigned , or cancelled
...
export const functions = [checkLowestStock,sendMonthlyOffers,autoAssignRider];
